...
Given a .csproj file located at //console:console.csproj with a TargetFramework of net8.0, invoking
`bazel build //console/console_publish` will result in
`bazel-bin/console/publish/net8.0/console.dll` that can be run with `dotnet exec console.dll`, and
...
def nuget_deps():
    nuget_fetch(
        name = "nuget",
        packages = {
            "Newtonsoft.Json/13.0.1": ["net8.0", "netstandard2.1"],
        },
        target_frameworks = ["net8.0", "netstandard2.1"],
...
1. .NET 6 is required to run rules_msbuild. You can still compile old frameworks though. This is 
   because rules_msbuild uses MSBuild 17 to build .NET 6 code, and MSBuild 17 targets net8.0 as a 
   TargetFramework.
